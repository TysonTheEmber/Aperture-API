...
/**
 * Utility class for smoothing keyframe transitions to eliminate camera jumps and create cinematic
 * camera animations.
 */
public class SmoothingUtils {

  /**
   * Apply cinematic smoothing to a keyframe to eliminate jumps. This sets the keyframe to use
   * SMOOTH path interpolation with BEZIER time curves.
   *
  ...
  /**
   * Apply bezier path smoothing to a keyframe for maximum control over movement curves. This
   * provides the smoothest possible transitions but requires more setup.
   *
   * @param keyframe The keyframe to smooth
  ...
  /**
   * Auto-smooth an entire camera path to eliminate all keyframe jumps. This analyzes the path and
   * applies appropriate smoothing to each segment.
   *
  ...
  private static EaseType determineOptimalEasing(
      Vector3f posDiff, Vector3f rotDiff, float fovDiff) {
