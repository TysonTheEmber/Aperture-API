...
public class RemotePathSearchScreen extends Screen {
    ...
    @Override
    protected void init() {
        int m = 20; int y = m; int rowH = 20; int s = 10; int avail = this.width - m * 2;
        // Fixed widths
        ...
        int x = m;
        NumberEditBox page = addRenderableWidget(new NumberEditBox(font, x, y, pageW, rowH, 1, Component.literal("page")));
        x += pageW + s;
        Component RELOAD = Component.translatable("gui.camera_anim.remote_path_search.reload");
        addRenderableWidget(Button.builder(RELOAD, b -> {
            if (REMOTE) {
                ClientPayloadSender.checkGlobalPath(Integer.parseInt(page.getValue()), 128);
            }
        }).pos(x, y).size(searchW, rowH).tooltip(Tooltip.create(Component.literal("Refresh list from server"))).build());
        x += searchW + s;
        pathBox = addRenderableWidget(new EditBox(font, x, y, flexW, rowH, Component.literal("path id")));
        x += flexW + s;
        addRenderableWidget(Button.builder(LOAD, b -> {
            if (REMOTE) {
                ClientPayloadSender.getGlobalPath(pathBox.getValue(), 0);
            }
        }).pos(x, y).size(loadBtnW, rowH).tooltip(Tooltip.create(Component.literal("Load animation with the given ID from server"))).build());
        x += loadBtnW + s;
        newIdBox = addRenderableWidget(new EditBox(font, x, y, flexW, rowH, Component.literal("new id")));
        newIdBox.setValue(CameraAnimIdeCache.getPath().getId());
        x += flexW + s;
        addRenderableWidget(Button.builder(SAVE, b -> {
            if (REMOTE) {
        ...
        }).pos(x, y).size(saveBtnW, rowH).tooltip(Tooltip.create(Component.literal("Save current path to server"))).build());
        x += saveBtnW + s;
        removeIdBox = addRenderableWidget(new EditBox(font, x, y, flexW, rowH, Component.literal("remove id")));
        x += flexW + s;
        addRenderableWidget(Button.builder(DELETE, b -> {
            if (REMOTE) {
                ClientPayloadSender.removeGlobalPath(removeIdBox.getValue());
            }
        }).pos(x, y).size(deleteBtnW, rowH).tooltip(Tooltip.create(Component.literal("Delete animation by ID on server"))).build());
        // Mode switch on far right
        addRenderableWidget(Button.builder(LOCAL_MODE, b -> Minecraft.getInstance().setScreen(new LocalPathSearchScreen()))
                .pos(this.width - m - modeW, y).size(modeW, rowH).tooltip(Tooltip.create(Component.literal("Switch to Local mode"))).build());

        // Filter full width under actions
        y += rowH + 20;
        filterBox = addRenderableWidget(new EditBox(font, m, y, this.width - m * 2, 18, Component.literal("filter")));
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        int idX = m; int modX = m + idW + 10; int timeX = modX + modW + 10;
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
        int m = 20; int headerY2 = (int)(m + 20 + 20 + 24);
        int top = headerY2 + 12;
        int rowH = 11;
        int visible = Math.max(0, (this.height - top - 40) / rowH);
