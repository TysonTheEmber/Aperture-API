...
/**
 * Core interpolation engine for the new path system.
 * Handles all position, orientation, and parameter interpolation.
 */
public class PathInterpolationEngine {
     ...
     */
    public static Vector3f interpolatePosition(PathModel.Segment current, PathModel.Segment next, 
                                               float localT, InterpolationType type, 
                                               int segmentIndex, List<PathModel.Segment> allSegments) {
        return switch (type) {
            ...
            case CATMULL_UNIFORM, CATMULL_CENTRIPETAL, CATMULL_CHORDAL -> 
                interpolateCatmullRom(current, next, localT, type.getCatmullAlpha(), segmentIndex, allSegments);
        };
    }
     ...
     */
    public static Quaternionf interpolateOrientation(PathModel.Segment current, PathModel.Segment next, 
                                                     float localT, boolean banking, float bankingStrength) {
     ...
     */
    public static float interpolateRoll(float currentRoll, float nextRoll, float localT, 
                                        EasingType easing, float bankingRoll, float rollMix) {
        float easedT = easing.apply(localT);
    ...
    private static Vector3f interpolateHermite(PathModel.Segment current, PathModel.Segment next, 
                                               float t, int segmentIndex, List<PathModel.Segment> allSegments) {
        ...
        return new Vector3f(p0).mul(h00)
                .add(new Vector3f(m0).mul(h10))
                .add(new Vector3f(p1).mul(h01))
                .add(new Vector3f(m1).mul(h11));
    }
    
    private static Vector3f calculateHermiteTangent(PathModel.Segment segment, int index, 
                                                    List<PathModel.Segment> allSegments, boolean incoming) {
        ...
        Vector3f next = index < allSegments.size() - 1 ? allSegments.get(index + 1).position : segment.position;
        ...
        if (incoming) {
            tangent = new Vector3f(inTangent).mul((1f - tension) * (1f + continuity) * (1f + bias) / 2f)
                    .add(new Vector3f(outTangent).mul((1f - tension) * (1f - continuity) * (1f - bias) / 2f));
        } else {
            tangent = new Vector3f(inTangent).mul((1f - tension) * (1f + continuity) * (1f - bias) / 2f)
                    .add(new Vector3f(outTangent).mul((1f - tension) * (1f - continuity) * (1f + bias) / 2f));
        }
    ...
    private static Vector3f interpolateBezier(PathModel.Segment current, PathModel.Segment next, float t) {
        ...
        return new Vector3f(p0).mul(mt3)
                .add(new Vector3f(p1).mul(3f * mt2 * t))
                .add(new Vector3f(p2).mul(3f * mt * t2))
                .add(new Vector3f(p3).mul(t3));
    }
    
    private static Vector3f interpolateCatmullRom(PathModel.Segment current, PathModel.Segment next, 
                                                  float t, float alpha, int segmentIndex, 
    ...
    /**
     * Enhanced Catmull-Rom spline with configurable alpha parameter
     * Based on the existing CatmullRom class but integrated here
     */
    private static Vector3f evaluateCatmullRom(float t, Vector3f p0, Vector3f p1, Vector3f p2, Vector3f p3, float alpha) {
        ...
        Vector3f a1 = new Vector3f(p1).sub(p0).div(t01)
                .sub(new Vector3f(p2).sub(p0).div(t01 + t12))
                .add(new Vector3f(p2).sub(p1).div(t12))
                .mul(t12);
                
        Vector3f a2 = new Vector3f(p2).sub(p1).div(t12)
        ...
        return new Vector3f(p1).mul(h00)
     ...
     */
    public static float calculateBankingRoll(PathModel.Segment current, PathModel.Segment next, 
                                             int segmentIndex, List<PathModel.Segment> allSegments, 
        ...
        Vector3f prev = segmentIndex > 0 ? 
            allSegments.get(segmentIndex - 1).position : current.position;
