...
public class CameraAnimIdeCache {
  ...
  private static float pickBezier(float length, Vector3f origin, Vector3f direction) {
    ...
    if (rightL <= length
        && Intersectionf.testRayAab(
            origin,
            direction,
            new Vector3f(right).sub(BEZIER_PICK_EXPAND, BEZIER_PICK_EXPAND, BEZIER_PICK_EXPAND),
            new Vector3f(right).add(BEZIER_PICK_EXPAND, BEZIER_PICK_EXPAND, BEZIER_PICK_EXPAND))) {
      length = rightL;
    ...
    if (leftL <= length
        && Intersectionf.testRayAab(
            origin,
            direction,
            new Vector3f(left).sub(BEZIER_PICK_EXPAND, BEZIER_PICK_EXPAND, BEZIER_PICK_EXPAND),
            new Vector3f(left).add(BEZIER_PICK_EXPAND, BEZIER_PICK_EXPAND, BEZIER_PICK_EXPAND))) {
      length = leftL;
  ...
  private static void pickPoint(float length, Vector3f origin, Vector3f direction) {
    ...
    for (Int2ObjectMap.Entry<CameraKeyframe> entry : PATH.getEntries()) {
      ...
      if (Intersectionf.testRayAab(
          origin,
          direction,
          new Vector3f(position).sub(POINT_PICK_EXPAND, POINT_PICK_EXPAND, POINT_PICK_EXPAND),
          new Vector3f(position).add(POINT_PICK_EXPAND, POINT_PICK_EXPAND, POINT_PICK_EXPAND))) {
        if (!(d < length)) {
          continue;
        }
  ...
  public static class MoveModeData {
    ...
    private boolean pickMoveModule(Vector3f origin, Vector3f direction, boolean leftClick) {
      ...
      if (leftClick) {
        ...
        float deadZone = 0.1f, l = 0.9f + 0.35f, w = 0.1f;

        // 快速检测是否在范围内
        if (!Intersectionf.testRayAab(
            origin,
            direction,
            new Vector3f(pos).sub(w, w, 0),
            new Vector3f(pos).add(deadZone + l, deadZone + l, deadZone + l))) {
          return false;
        }
        ...
        if (Intersectionf.intersectRayAab(
            origin,
            direction,
            new Vector3f(pos).add(deadZone, -w, -w),
            new Vector3f(pos).add(deadZone + l, w, w),
            resultPos)) {
          min = resultPos.x;
          result = true;
        ...
        if (Intersectionf.intersectRayAab(
                origin,
                direction,
                new Vector3f(pos).add(-w, deadZone, -w),
                new Vector3f(pos).add(w, deadZone + l, w),
                resultPos)
            && min > resultPos.x) {
          min = resultPos.x;
          result = true;
        ...
        if (Intersectionf.intersectRayAab(
                origin,
                direction,
                new Vector3f(pos).add(-w, -w, deadZone),
                new Vector3f(pos).add(w, w, deadZone + l),
                resultPos)
            && min > resultPos.x) {
          min = resultPos.x;
          result = true;
        ...
        float spacing = 0.2f, size = 0.3f;
        // xy
        if (Intersectionf.intersectRayAab(
                origin,
                direction,
                new Vector3f(pos).add(spacing, spacing, -w),
                new Vector3f(pos).add(size + spacing, size + spacing, w),
                resultPos)
            && min > resultPos.x) {
          min = resultPos.x;
          result = true;
        ...
        if (Intersectionf.intersectRayAab(
                origin,
                direction,
                new Vector3f(pos).add(spacing, -w, spacing),
                new Vector3f(pos).add(size + spacing, w, size + spacing),
                resultPos)
            && min > resultPos.x) {
          min = resultPos.x;
          result = true;
        ...
        if (Intersectionf.intersectRayAab(
                origin,
                direction,
                new Vector3f(pos).add(-w, spacing, spacing),
                new Vector3f(pos).add(w, size + spacing, size + spacing),
                resultPos)
            && min > resultPos.x) {
          min = resultPos.x;
          result = true;
    ...
    private boolean pickRotateModule(Vector3f origin, Vector3f direction) {
      ...
      float r = 1.2f;
      // Match clickable thickness to rendered band (halfWidth ~0.06). Picking feels better slightly
      // larger than render.
      float ringHalfWidth = 0.06f;
      float pickHalfWidth =
          ringHalfWidth * 2.5f + 0.02f; // widen pick to roughly align with perceived band thickness
      ...
      java.util.function.BiFunction<Vector3f, Character, Float> testAxis =
          (c, axis) -> {
            ...
            float d =
                (axis == 'x')
                    ? (float) Math.sqrt(hit.y * hit.y + hit.z * hit.z)
                    : (axis == 'y')
                        ? (float) Math.sqrt(hit.x * hit.x + hit.z * hit.z)
      ...
      for (Candidate c : cands) {
        if (bestCand == null
            || c.dev < bestCand.dev
            || (c.dev == bestCand.dev && Math.abs(c.t) < Math.abs(bestCand.t))) {
          bestCand = c;
        }
      }
      ...
      switch (best) {
        case RX -> {
          startAngle = (float) Math.toDegrees(Math.atan2(hit.z, hit.y));
          currentAxis = 'X';
        }
        case RY -> {
          startAngle = (float) Math.toDegrees(Math.atan2(hit.x, hit.z));
          currentAxis = 'Y';
        }
        case RZ -> {
          startAngle = (float) Math.toDegrees(Math.atan2(hit.y, hit.x));
          currentAxis = 'Z';
        }
      }
      currentDeltaDeg = 0f;
    ...
    private void move() {
      ...
      switch (moveType) {
        ...
        case RX, RY, RZ -> {
          ...
          float t =
              Intersectionf.intersectRayPlane(
                  origin.x, origin.y, origin.z, view.x, view.y, view.z, center.x, center.y,
                  center.z, nx, ny, nz, 1e-6f);
  ...
  public static boolean isRotating() {
    return MODE == Mode.ROTATE && MOVE_DATA.moveType == MoveType.RX
        || MOVE_DATA.moveType == MoveType.RY
        || MOVE_DATA.moveType == MoveType.RZ;
  }
