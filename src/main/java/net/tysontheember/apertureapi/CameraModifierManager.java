...
public class CameraModifierManager {
  ...
  // Positive order (H -> L)
  private static final HashMap<String, Modifier>[] positiveModifiers =
      new HashMap[] {modifiersH, modifiersL};
  // Negative order (L -> H)
  private static final HashMap<String, Modifier>[] negativeModifiers =
      new HashMap[] {modifiersL, modifiersH};
  ...
  private static void applyPlayerOrderModifier() {
    ...
    for (int i = playerOrder.size() - 1; i >= 0; i--) {
      ...
      if (modifier == null
          || !modifier.isStateEnabledOr(ModifierStates.ENABLE)
          || !modifier.isEffective) {
        continue;
      }

      // Skip if no position/rotation/FOV flags are enabled
      if (!modifier.isStateEnabledOr(
          ModifierStates.POS_ENABLED | ModifierStates.ROT_ENABLED | ModifierStates.FOV_ENABLED)) {
        continue;
      }
  ...
  private static Modifier getEffectiveModifierFromMap(HashMap<String, Modifier> map) {
    for (Modifier modifier : map.values()) {
      if (modifier.isStateEnabledOr(ModifierStates.ENABLE)
          && modifier.isEffective
          && (modifier.isStateEnabledOr(
              ModifierStates.POS_ENABLED
                  | ModifierStates.ROT_ENABLED
                  | ModifierStates.FOV_ENABLED))) {
        return modifier;
      }
    }
  ...
  private static void applyModifyToRot(float partialTick, float yRot) {
    ...
    if (isStateEnabledOr(ModifierStates.GLOBAL_MODE_ENABLED)) {
      // 全局模式，不应用玩家旋转
      if (isOldStateEnabledAnd(
          ModifierStates.ENABLE | ModifierStates.ROT_ENABLED | ModifierStates.LERP)) {
        // 如果上次开启了旋转，则要计算插值
        rot =
            new Vector3f(
  ...
  private static void applyModifyToPos(float partialTick, float yRot, Entity entity) {
    ...
    if (isStateEnabledOr(ModifierStates.GLOBAL_MODE_ENABLED)) {
      // 全局模式
      if (isOldStateEnabledAnd(
          ModifierStates.GLOBAL_MODE_ENABLED
              | ModifierStates.ENABLE
              | ModifierStates.POS_ENABLED
              | ModifierStates.LERP)) {
        // 如果上次开启了全局模式，则要计算插值
        pos =
      ...
      // 局部模式
      if (isOldStateEnabledAnd(
              ModifierStates.ENABLE | ModifierStates.POS_ENABLED | ModifierStates.LERP)
          && !isOldStateEnabledOr(ModifierStates.GLOBAL_MODE_ENABLED)) {
        // 如果上次是局部模式，则计算插值
        pos =
            new Vector3d(
  ...
  /**
   * Create a modifier that returns the already created modifier if the provided name has already
   * been used. Although it's not mandatory to use modid as the name, considering there is only one
   * camera, usually one modifier is sufficient.
   *
   * @param modID You mod's id, but this is not mandatory.
   * @param high True for high priority. More likely to be used, but there is no guarantee that it
  ...
  /**
   * Create a background modifier. All background modifier will be applied.
   *
   * @param modID You mod's id, but this is not mandatory.
  ...
  public static class Modifier implements ICameraModifier {
    ...
    public Modifier aimAt(double x, double y, double z) {
      ...
      rot.x =
          (float) Math.acos(Math.sqrt(aim.x * aim.x + aim.z * aim.z) / aim.length())
              * Mth.RAD_TO_DEG
              * (aim.y < 0 ? 1 : -1);
  ...
  public static void modifyFov(ViewportEvent.ComputeFov event) {
    ...
    if (isOldStateEnabledAnd(
        ModifierStates.ENABLE | ModifierStates.FOV_ENABLED | ModifierStates.LERP)) {
  ...
  public static void modifyFirstPersonHand(RenderHandEvent event) {
    ...
    if (!isStateEnabledAnd(ModifierStates.ENABLE | ModifierStates.FIRST_PERSON_ARM_FIXED)
        || isStateEnabledOr(ModifierStates.GLOBAL_MODE_ENABLED)) {
      return;
    }
