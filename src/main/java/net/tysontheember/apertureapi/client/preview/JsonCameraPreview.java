...
/**
 * Minimal client-only JSON camera path preview. Supports: linear position, linear rotation
 * (yaw/pitch) or lookAt per-keyframe, keyframed FOV. Falls back gracefully if fields are missing.
 */
@Mod.EventBusSubscriber(modid = ApertureAPI.MODID, value = Dist.CLIENT)
public class JsonCameraPreview {
  private static final ICameraModifier PREVIEW_MOD =
      CameraModifierManager.createModifier(ApertureAPI.MODID + "_json_preview", true)
          .enableGlobalMode()
          .enablePos()
          .enableRotation()
          .enableFov()
  ...
  private static Vector3f slerpDir(Vector3f a, Vector3f b, float t) {
    ...
    if (dot > 0.9995f) {
      ...
      return new Vector3f(Mth.lerp(t, v0.x, v1.x), Mth.lerp(t, v0.y, v1.y), Mth.lerp(t, v0.z, v1.z))
          .normalize();
    }
