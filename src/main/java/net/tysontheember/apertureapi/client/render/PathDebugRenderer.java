...
/**
 * Lightweight debug path renderer that draws curve lines and optional velocity coloring.
 * Intended for edit/preview mode live visualization.
 */
public final class PathDebugRenderer {
    ...
    public static void render(GlobalCameraPath path, PoseStack.Pose pose, MultiBufferSource.BufferSource buffers, Vec3 cameraPos) {
        ...
        for (int i = 1; i < points.size(); i++) {
            ...
            Vector3f p1 = new Vector3f(pre.getPos()).sub((float) cameraPos.x, (float) cameraPos.y, (float) cameraPos.z);
            Vector3f p2 = new Vector3f(next.getPos()).sub((float) cameraPos.x, (float) cameraPos.y, (float) cameraPos.z);
    ...
    private static void eval(GlobalCameraPath path, int segIndex, float t, CameraKeyframe pre, CameraKeyframe next, Vec3 cam, Vector3f out) {
        ...
        switch (mode) {
            case LINEAR -> InterpolationMath.line(t, new Vector3f(pre.getPos()).sub((float) cam.x, (float) cam.y, (float) cam.z), new Vector3f(next.getPos()).sub((float) cam.x, (float) cam.y, (float) cam.z), out);
            case COSINE -> {
                float tc = (1.0f - (float) Math.cos(Math.PI * t)) * 0.5f;
                InterpolationMath.line(tc, new Vector3f(pre.getPos()).sub((float) cam.x, (float) cam.y, (float) cam.z), new Vector3f(next.getPos()).sub((float) cam.x, (float) cam.y, (float) cam.z), out);
            }
            case SMOOTH -> InterpolationMath.catmullRom(t,
                    ...
                    new Vector3f(p3).sub((float) cam.x, (float) cam.y, (float) cam.z), out);
            case CATMULL_UNIFORM -> CatmullRom.eval(t,
                    new Vector3f(p0).sub((float) cam.x, (float) cam.y, (float) cam.z),
                    new Vector3f(pre.getPos()).sub((float) cam.x, (float) cam.y, (float) cam.z),
                    new Vector3f(next.getPos()).sub((float) cam.x, (float) cam.y, (float) cam.z),
                    new Vector3f(p3).sub((float) cam.x, (float) cam.y, (float) cam.z), 0.0f, out);
            case CATMULL_CENTRIPETAL -> CatmullRom.eval(t,
                    new Vector3f(p0).sub((float) cam.x, (float) cam.y, (float) cam.z),
                    new Vector3f(pre.getPos()).sub((float) cam.x, (float) cam.y, (float) cam.z),
                    new Vector3f(next.getPos()).sub((float) cam.x, (float) cam.y, (float) cam.z),
                    new Vector3f(p3).sub((float) cam.x, (float) cam.y, (float) cam.z), 0.5f, out);
            case CATMULL_CHORDAL -> CatmullRom.eval(t,
                    new Vector3f(p0).sub((float) cam.x, (float) cam.y, (float) cam.z),
                    new Vector3f(pre.getPos()).sub((float) cam.x, (float) cam.y, (float) cam.z),
                    new Vector3f(next.getPos()).sub((float) cam.x, (float) cam.y, (float) cam.z),
                    new Vector3f(p3).sub((float) cam.x, (float) cam.y, (float) cam.z), 1.0f, out);
            case BEZIER -> pre.getPathBezier().interpolate(t,
                    new Vector3f(pre.getPos()).sub((float) cam.x, (float) cam.y, (float) cam.z),
                    new Vector3f(next.getPos()).sub((float) cam.x, (float) cam.y, (float) cam.z), out);
            case STEP -> out.set(new Vector3f(pre.getPos()).sub((float) cam.x, (float) cam.y, (float) cam.z));
        }
    }
