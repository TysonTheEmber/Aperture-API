...
/**
 * Datapack JSON loader + in-memory registry for camera paths.
 * Loads files at data/apertureapi/camera_paths/*.json
 */
public class CameraPathRegistry extends SimpleJsonResourceReloadListener {
    ...
    protected void apply(Map<ResourceLocation, JsonElement> input, ResourceManager resourceManager, ProfilerFiller profiler) {
        ...
        LOGGER.info("CameraPathRegistry: scanning folder '{}' with {} resource(s)", FOLDER, input.size());
        ...
        input.forEach((rl, json) -> {
            try {
                ...
                if (temp.containsKey(def.id)) {
                    LOGGER.warn("Duplicate camera path id '{}' encountered. Keeping the last one ({}).", def.id, rl);
                }
                temp.put(def.id, def);
            } catch (Exception e) {
    ...
    private static boolean validate(CameraPathDef def, ResourceLocation rl) {
        ...
        for (CameraPathDef.KeyframeDef kf : def.keyframes) {
            if (kf == null) { LOGGER.warn("'{}' in {}: keyframe {} is null", def.id, rl, idx); return false; }
            if (kf.pos == null) { LOGGER.warn("'{}' in {}: keyframe {} missing pos", def.id, rl, idx); return false; }
            if (kf.lookAt == null && kf.rot == null) {
                LOGGER.debug("'{}' in {}: keyframe {} has no rot or lookAt; rotation will be resolved at runtime", def.id, rl, idx);
            }
            if (kf.time < last) {
                LOGGER.warn("'{}' in {}: keyframe {} time {} < previous {} (must be non-decreasing)", def.id, rl, idx, kf.time, last);
                return false;
            }
            last = kf.time;
            idx++;
    ...
    public static Map<String, CameraPathDef> all() { return PATHS; }
    public static CameraPathDef get(String id) { return PATHS.get(id); }
    public static boolean exists(String id) { return PATHS.containsKey(id); }
}
