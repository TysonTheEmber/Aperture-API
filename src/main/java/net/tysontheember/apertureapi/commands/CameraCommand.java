...
public class CameraCommand {
    private static final SuggestionProvider<CommandSourceStack> PATH_SUGGESTER = (ctx, b) -> suggestPathIds(ctx, b);
    ...
    public static void register(CommandDispatcher<CommandSourceStack> d, String root) {
        d.register(Commands.literal(root)
            ...
            .then(Commands.literal("play").requires(s -> CommandUtils.permCheck(s, 2))
            ...
            .then(Commands.literal("export").requires(s -> CommandUtils.permCheck(s, 2))
                .then(Commands.argument("name", StringArgumentType.word()).suggests(PATH_SUGGESTER)
                    .then(Commands.argument("file", StringArgumentType.word()).executes(CameraCommand::exportAs))))
        );
    }
    ...
    private static int play(CommandContext<CommandSourceStack> ctx) {
        ...
        CommandUtils.msgInfo(ctx.getSource(), "Playing '" + name + "' (speed=" + speed + ", loop=" + loop + ")");
        return 1;
    }
    ...
    private static java.util.concurrent.CompletableFuture<Suggestions> suggestSpeeds(CommandContext<CommandSourceStack> ctx, SuggestionsBuilder b) {
        for (String s : new String[]{"0.5","1.0","2.0"}) { if (s.startsWith(b.getRemaining())) b.suggest(s); }
    ...
    private static CompletableFuture<Suggestions> suggestInterpolations(CommandContext<CommandSourceStack> ctx, SuggestionsBuilder b) {
        String[] modes = new String[]{
                "linear","smooth","bezier","step","cosine",
                "catmull:uniform","catmull:centripetal","catmull:chordal"
        };
    ...
    private static int stopSelf(CommandContext<CommandSourceStack> ctx) {
        try {
            ...
            net.tysontheember.apertureapi.common.network.ServerPayloadSender.send("stopCamera", new net.minecraft.nbt.CompoundTag(), player);
            CommandUtils.msgInfo(ctx.getSource(), "Stopped camera for " + player.getGameProfile().getName());
            return 1;
        } catch (Exception e) {
    ...
    private static int stopTarget(CommandContext<CommandSourceStack> ctx) {
        try {
            var target = net.minecraft.commands.arguments.EntityArgument.getPlayer(ctx, "target");
            net.tysontheember.apertureapi.common.network.ServerPayloadSender.send("stopCamera", new net.minecraft.nbt.CompoundTag(), target);
            CommandUtils.msgInfo(ctx.getSource(), "Stopped camera for " + target.getGameProfile().getName());
            return 1;
        } catch (Exception e) {
    ...
    private static int setInterpolation(CommandContext<CommandSourceStack> ctx) {
        ...
        GlobalCameraPath path = data.getPath(name);
        if (path == null) { CommandUtils.msgError(ctx.getSource(), "Unknown path: '" + name + "'"); return 0; }
        net.tysontheember.apertureapi.common.animation.PathInterpolator interp = switch (mode.toLowerCase()) {
            ...
            case "catmull:uniform" -> net.tysontheember.apertureapi.common.animation.PathInterpolator.CATMULL_UNIFORM;
            case "catmull:centripetal" -> net.tysontheember.apertureapi.common.animation.PathInterpolator.CATMULL_CENTRIPETAL;
            case "catmull:chordal" -> net.tysontheember.apertureapi.common.animation.PathInterpolator.CATMULL_CHORDAL;
            default -> null;
        };
        if (interp == null) { CommandUtils.msgError(ctx.getSource(), "Unknown interpolation mode: '" + mode + "'"); return 0; }
