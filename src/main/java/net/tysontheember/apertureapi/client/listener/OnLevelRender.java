...
public class OnLevelRender {
  ...
  private static void renderCameraAnimIdePath(RenderLevelStageEvent event) {
    ...
    if (net.tysontheember.apertureapi.client.render.PathDebugRenderer.showVelocityColor
      ...
      net.tysontheember.apertureapi.client.render.PathDebugRenderer.render(
          getPath(), last, bufferSource, p);
    }
  ...
  private static final ICameraModifier IDE_MODIFIER =
      CameraModifierManager.createModifier(ApertureAPI.MODID + "_ide", false)
          .enableFov()
          .enablePos()
          .enableRotation()
          .enableGlobalMode();
  ...
  private static final ICameraModifier PLAYER_MODIFIER =
      CameraModifierManager.createModifier(ApertureAPI.MODID + "_player", true)
          .enableFov()
          .enablePos()
          .enableRotation()
          .enableGlobalMode();
  ...
  private static void renderLines(
      SelectedPoint selected, MultiBufferSource.BufferSource bufferSource, PoseStack.Pose pose) {
  ...
  private static void renderNativeCenter(PoseStack.Pose pose, VertexConsumer buffer) {
    if (getPath().isNativeMode()) {
      addLine(
          buffer,
          pose,
          getNativePos().add(0.5f, 0.1f, 0.5f, V_CACHE_3).sub(CAMERA_CACHE),
          getNativePos().add(-0.5f, 0.1f, 0.5f, V_CACHE_2).sub(CAMERA_CACHE),
          0xffe76767);
      addLine(
          buffer,
          pose,
      ...
      addLine(
          buffer,
          pose,
          getNativePos().add(0, 0.1f, 0, V_CACHE_3).sub(CAMERA_CACHE),
          getNativePos().add(rot, V_CACHE_2).sub(CAMERA_CACHE),
          0xff00ff00);
    }
  }
    ...
    if (!PREVIEW) {
      if (animator.prepareCameraInfo(IDE_POS, IDE_ROT, IDE_FOV)) {
        ...
        addLine(
            buffer,
            pose,
            V_CACHE_1.set(-0.15f, -0.1f, -0.1f).rotate(Q_CACHE).add(IDE_POS).sub(CAMERA_CACHE),
            V_CACHE_2.set(0.15f, -0.1f, -0.1f).rotate(Q_CACHE).add(IDE_POS).sub(CAMERA_CACHE),
            0xff000000);
        addLine(
            buffer,
            pose,
            V_CACHE_1.set(-0.15f, -0.1f, 0f).rotate(Q_CACHE).add(IDE_POS).sub(CAMERA_CACHE),
            V_CACHE_2.set(0.15f, -0.1f, 0f).rotate(Q_CACHE).add(IDE_POS).sub(CAMERA_CACHE),
            0xff000000);
        addLine(
            buffer,
            pose,
        ...
        addLine(
            buffer,
            pose,
            V_CACHE_1.set(0.5f, 0.28125f, 0.3f).rotate(Q_CACHE).add(IDE_POS).sub(CAMERA_CACHE),
            V_CACHE_2.set(-0.5f, 0.28125f, 0.3f).rotate(Q_CACHE).add(IDE_POS).sub(CAMERA_CACHE),
            0xff000000);
        addLine(
            buffer,
            pose,
  ...
  private static void renderTrackLine(
      SelectedPoint selected, VertexConsumer buffer, PoseStack.Pose last) {
      ...
      switch (p2.getPathInterpolator()) {
        ...
        case BEZIER -> {
          ...
          addBezierLine(
              ...
              last,
              v1,
              V_CACHE_3.set(controller.getLeft()).sub(CAMERA_CACHE),
              V_CACHE_4.set(controller.getRight()).sub(CAMERA_CACHE),
              v2,
              0xffffffff);
        }
  ...
  private static void renderMoveLine(
      SelectedPoint selected, VertexConsumer buffer, PoseStack.Pose last) {
    ...
    int xColor = X_COLOR, yColor = Y_COLOR, zColor = Z_COLOR;
  ...
  // 使用vCache1、vCache2
  private static void renderBezierLine(
      SelectedPoint selected, VertexConsumer buffer, PoseStack.Pose last) {
    int selectedTime = selected.getPointTime();
    GlobalCameraPath track = getPath();
  ...
  private static void renderFilledBox(
      SelectedPoint selected, MultiBufferSource.BufferSource bufferSource, PoseStack.Pose last) {
  ...
  // Uses V_CACHE_1
  private static void renderBezierPoint(
      SelectedPoint selected, VertexConsumer buffer, PoseStack.Pose last) {
    int selectedTime = selected.getPointTime();
    ...
    Vec3BezierController controller = selectedPoint.getPathBezier();
    addPoint(
        buffer, last, V_CACHE_1.set(controller.getLeft()).sub(CAMERA_CACHE), 0.05f, 0x7f98FB98);
    addPoint(
        buffer, last, V_CACHE_1.set(controller.getRight()).sub(CAMERA_CACHE), 0.05f, 0x7f98FB98);
  }
  ...
  private static void renderSelectedPoint(
    ...
    switch (selected.getControl()) {
      case LEFT, RIGHT -> addPoint(
          buffer, last, V_CACHE_1.set(pos).sub(CAMERA_CACHE), 0.07f, SELECTED_COLOR_TRANSPARENT);
      case NONE -> addPoint(
          buffer, last, V_CACHE_1.set(pos).sub(CAMERA_CACHE), 0.12f, SELECTED_COLOR_TRANSPARENT);
    }
  }
  ...
  // Uses V_CACHE_1
  private static void renderMoveSlice(
      SelectedPoint selected, VertexConsumer buffer, PoseStack.Pose last) {
    Vector3f pos = selected.getPosition();
    ...
    pos = V_CACHE_1.set(pos).sub(CAMERA_CACHE);
    int xyColor = Z_COLOR_TRANSPARENT, yzColor = X_COLOR_TRANSPARENT, xzColor = Y_COLOR_TRANSPARENT;

    // Keep default axis colors (do not switch to blue)
    switch (getMoveMode().getMoveType()) {
      case XY -> {
        /* keep xyColor as Z_COLOR_TRANSPARENT */
      }
      case YZ -> {
        /* keep yzColor as X_COLOR_TRANSPARENT */
      }
      case XZ -> {
        /* keep xzColor as Y_COLOR_TRANSPARENT */
      }
    }
  ...
  private static void addLine(
      VertexConsumer buffer, PoseStack.Pose pose, Vector3f pos1, Vector3f pos2, int color) {
    Vector3f normalize = V_CACHE_5.set(pos2).sub(pos1).normalize();
    Matrix3f matrix3f = pose.normal();
    Matrix4f matrix4f = pose.pose();
    buffer
        .vertex(matrix4f, pos1.x, pos1.y, pos1.z)
        .color(color)
        .normal(matrix3f, normalize.x, normalize.y, normalize.z)
        .endVertex();
    buffer
        .vertex(matrix4f, pos2.x, pos2.y, pos2.z)
        .color(color)
        .normal(matrix3f, normalize.x, normalize.y, normalize.z)
        .endVertex();
  }
  ...
  private static void addSmoothLine(
  ...
  // Uses V_CACHE_6 and V_CACHE_7
  private static void addCatmullAlphaLine(
      VertexConsumer buffer,
      PoseStack.Pose pose,
      Vector3f pre,
      Vector3f p1,
  ...
  // Uses V_CACHE_6 and V_CACHE_7
  private static void addBezierLine(
      VertexConsumer buffer,
      PoseStack.Pose pose,
      Vector3f p1,
      Vector3f c1,
  ...
  private static void addPoint(
      VertexConsumer buffer, PoseStack.Pose pose, Vector3f pos, float size, int color) {
  ...
  private static void renderRotateGizmo(
      SelectedPoint selected, VertexConsumer buffer, PoseStack.Pose last) {
    int selectedTime = selected.getPointTime();
    ...
    // Strong highlight: draw multiple concentric opaque rings on the picked axis (use axis color,
    // not blue)
    switch (getMoveMode().getMoveType()) {
      case RX -> {
  ...
  // Render a thick ring band for the actively used rotation axis using quads (appears
  // bolder/thicker)
  private static void renderRotateBand(
      SelectedPoint selected, VertexConsumer buffer, PoseStack.Pose last) {
    switch (getMoveMode().getMoveType()) {
      case RX, RY, RZ -> {
        int selectedTime = selected.getPointTime();
  ...
  private static void renderMovePlaneOutline(
      SelectedPoint selected, VertexConsumer buffer, PoseStack.Pose last) {
  ...
  // Draw a simple camera frustum (cone/pyramid) for the selected keyframe while actively adjusting
  // it
  private static void renderCameraConeDuringAdjust(
      SelectedPoint selected, VertexConsumer buffer, PoseStack.Pose pose) {
    int selectedTime = selected.getPointTime();
    if (selectedTime < 0) return;

    // Only when actively adjusting (dragging move/rotate), and only for the main keyframe (not
    // Bezier handles)
    switch (getMoveMode().getMoveType()) {
      case NONE -> {
        return;
    ...
    // Orientation from keyframe rotation (degrees -> radians), match existing YXZ order and yaw
    // inversion
