...
public class CameraModifierScreen extends Screen {
  ...
  private static class ComponentSCRollWidget extends AbstractWidget {
    ...
    public ComponentSCRollWidget(
        int x, int y, int width, int height, Component message, List<MutableComponent> list) {
      ...
      if (list.size() <= maxCount) {
        ...
        paneLength =
            (int)
                Math.max(3, Math.ceil((float) (height - 8) / Math.max(list.size() - maxCount, 1)));
      }
    }
      ...
      if (mouseX > getX() + 2
        ...
        if (contents instanceof TranslatableContents c) {
          guiGraphics.renderComponentTooltip(
              font,
              List.of(
                  Component.translatable(c.getKey() + ".tip1"),
                  Component.translatable(c.getKey() + ".tip2")),
              mouseX,
              mouseY);
        }
      }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
      if (this.active && this.visible && button == 0) {
        ...
        int i =
            Math.min((int) (mouseY - y) / 11, maxCount - 1) + Math.min((int) roll, list.size() - 1);
  ...
  private static class NormalModifierWidget extends ComponentSCRollWidget {
    ...
    private static NormalModifierWidget create(int x, int y, int width, int height) {
      ...
      return new NormalModifierWidget(
          x,
          y,
          width,
          height,
          Component.translatable("camera_modifier_screen.normal_modifier"),
          ids,
          components);
    }

    private static void loadModifier(List<String> ids, List<MutableComponent> components) {
      HashMap<String, CameraModifierManager.Modifier> h =
          new HashMap<>(CameraModifierManager.getModifiersH());
      HashMap<String, CameraModifierManager.Modifier> l =
          new HashMap<>(CameraModifierManager.getModifiersL());
  ...
  private static class PlayerOrderModifierWidget extends ComponentSCRollWidget {
    ...
    public PlayerOrderModifierWidget(
        int x, int y, int width, int height, Component message, List<MutableComponent> list) {
      super(x, y, width, height, message, list);
    }
    ...
    private static PlayerOrderModifierWidget create(int x, int y, int width, int height) {
      ...
      return new PlayerOrderModifierWidget(
          x,
          y,
          width,
          height,
          Component.translatable("camera_modifier_screen.player_order"),
          components);
    }
  ...
  private static class BackgroundModifierWidget extends ComponentSCRollWidget {
    ...
    public BackgroundModifierWidget(
        int x,
        int y,
        int width,
        int height,
    ...
    private static BackgroundModifierWidget create(int x, int y, int width, int height) {
      ...
      return new BackgroundModifierWidget(
          x,
          y,
          width,
          height,
          Component.translatable("camera_modifier_screen.background_modifier"),
          ids,
          components);
    }

    private static void loadModifier(List<String> ids, List<MutableComponent> components) {
      HashMap<String, CameraModifierManager.Modifier> b =
          new HashMap<>(CameraModifierManager.getModifiersB());
  ...
  private static class PlayerRemovedModifierWidget extends ComponentSCRollWidget {
    ...
    public PlayerRemovedModifierWidget(
        int x, int y, int width, int height, Component message, List<MutableComponent> list) {
      super(x, y, width, height, message, list);
    }
    ...
    private static PlayerRemovedModifierWidget create(int x, int y, int width, int height) {
      ...
      return new PlayerRemovedModifierWidget(
          x,
          y,
          width,
          height,
          Component.translatable("camera_modifier_screen.removed_modifier"),
          components);
    }
